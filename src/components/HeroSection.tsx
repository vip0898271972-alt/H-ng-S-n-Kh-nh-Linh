  ...
  return (
    <section id="top" className="relative pt-4 pb-10 md:pt-8 md:pb-16 text-center px-4 overflow-hidden">
      ...
      <div className="max-w-md mx-auto">
        ...
        <div className="bg-white/90 backdrop-blur-xs rounded-2xl p-3 border border-[#E8DFC8] shadow-sm mb-6 max-w-[340px] mx-auto">
          <p className="text-[10px] uppercase tracking-widest text-stone-500 mb-2 font-semibold">
            {timeLeft.isPast ? 'Ngày hạnh phúc đã trọn vẹn' : 'Đếm ngược ngày chung đôi'}
          </p>
          <div className="grid grid-cols-4 gap-1 text-center">
            <div className="bg-[#FAF7F2] py-1.5 rounded-lg border border-stone-100">
              <span className="block font-serif-elegant text-xl font-bold text-[#9B2C2C] leading-none">
                {String(timeLeft.days).padStart(2, '0')}
              </span>
              <span className="text-[9px] text-stone-500 uppercase">Ngày</span>
            </div>
            <div className="bg-[#FAF7F2] py-1.5 rounded-lg border border-stone-100">
              <span className="block font-serif-elegant text-xl font-bold text-[#9B2C2C] leading-none">
                {String(timeLeft.hours).padStart(2, '0')}
              </span>
              <span className="text-[9px] text-stone-500 uppercase">Giờ</span>
            </div>
            <div className="bg-[#FAF7F2] py-1.5 rounded-lg border border-stone-100">
              <span className="block font-serif-elegant text-xl font-bold text-[#9B2C2C] leading-none">
                {String(timeLeft.minutes).padStart(2, '0')}
              </span>
              <span className="text-[9px] text-stone-500 uppercase">Phút</span>
            </div>
            <div className="bg-[#FAF7F2] py-1.5 rounded-lg border border-stone-100">
              <span className="block font-serif-elegant text-xl font-bold text-[#9B2C2C] leading-none">
                {String(timeLeft.seconds).padStart(2, '0')}
              </span>
              <span className="text-[9px] text-stone-500 uppercase">Giây</span>
            </div>
          </div>
        </div>

        {/* Primary Action Buttons */}
        <div className="flex items-center justify-center gap-3 max-w-[340px] mx-auto">
          <button
            onClick={() => {
              onScrollToRsvp();
              fireWeddingConfetti();
            }}
            className="flex-1 flex items-center justify-center gap-1.5 py-3 px-4 rounded-full bg-gradient-to-r from-[#9B2C2C] to-[#C53030] hover:from-[#801F1F] hover:to-[#9B2C2C] text-white text-xs font-semibold shadow-md hover:shadow-lg active:scale-95 transition-all cursor-pointer"
          >
          ...
          <a
            ...
            className="flex items-center justify-center gap-1 py-3 px-4 rounded-full bg-white hover:bg-stone-50 text-[#8C6D46] border border-[#D4AF37]/50 text-xs font-semibold shadow-xs active:scale-95 transition-all"
          >
